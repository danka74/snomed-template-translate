...
var combineTerms = function(total, term, index, length) {
    if(index == 0) {
        return total + term;
    }
    if(index == length - 1) {
...
const newTerms = concepts.map(concept => {
    ...
    const statedCount = concept.groups.reduce((total, group) => {
        let rel = group.rels.find(r => r.typeId == 766952006);
        if(rel) {
    ...
    if(concept.semtag === "clinical drug") {
        const wordList = concept.groups.reduce((total, group) => {
            const rel = group.rels.find(r => r.typeId == 127489000)
            const relPrecisely = group.rels.find(r => r.typeId == 762949000)
            if(rel || relPrecisely) {
                let phrase = "";
...
newTerms.forEach(element => {
    console.log(element.sctid + "\t" + element.fsn + " \t" + element.newTerm);
});